...
)
{
    ...
    ///回声服务器（应该是有回复）
    /// 下面服务器echo_bridge，echo接收数据，又把相同内容发送出去
...
struct my_example_object
{
    ...
    */
    ...
    ///所有通过dlib::bridge 发送的数据都必须可序列化（即存在serialize()和deserialize()）
    /// 可以将对象转换到比特流和其逆过程
...
void deserialize (my_example_object& item, std::istream& in)
{
    ...
    */
    ...
    /// 这意味着生deserialize()需要消耗serialize（）产生的同等数量的字节流
...
)
{
    ...
    */
    ...
    ///bridge_status 用于产生 建立或者关闭监听的信号
    /// 可能的实现是：bridge状态发生变化时总是在发出信号，但是若没有设置接收该信号的数据类型的管道，该数据就会被过滤
    /// 接收的管道可以是type_safe_union包含bridge_status的管道，也可以是bridge_status的管道
    ...
    /// type_safe_union 中也包含判断数据是否是某一类型的方法
