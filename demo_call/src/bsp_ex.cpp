...
///整体同步并行计算模型的使用
/// 简便的建立一系列运行在不同电脑上的进程，来合作计算
/// 搜索方法是嵌套网格搜索
///
/// 执行以下步骤来运行此程序
/// 打开三个命令行，工作路径均为当前程序的可执行文件路径
/// 分别执行
/// ./bsp_ex -l12345
///  ./bsp_ex -l12346
/// ./bsp_ex localhost:12345 localhost:12346
///
///
/// 具体解决的问题是在一条双曲线上寻找值最小的横坐标
/// 这里的思路是分成三个进程
/// 进程0 分发任务，将一个区域A发送给进程1 和进程2
/// 进程1 和进程2 根据各自的进程号，将A分成A1、A2，然后各自在某一个区域Ax上计算Ax上线段上的最小值
/// ...具体的方法是固定距离采样，然后逐点计算最小值
/// 进程0收集进程1和进程2的计算结果，和上一次保留的结果P,进行比较，取值最小的点m
/// 然后以m为中心，以A的宽度的一半重新确定一个区域B，重复以上步骤。直到确定的区域的宽度小于某一数值，即满足某一精度
...
void bsp_job_node_0 (bsp_context& bsp, double& min_value, double& optimal_x)
{
    /// 作为其他控制器的节点，从其他节点收集数据
    /// 每个节点有节点id，由bsp_connect生成的节点id为0
    /// 每个任务以bsp_context作为第一个参数，该对象为bsp任务交互的接口
...
void bsp_job_other_nodes (bsp_context& bsp, long grid_resolution)
{
    ...
    while (bsp.try_receive(left))
    {
        ...
        {
            std::cout<<std::this_thread::get_id()<<"["<<best_val<<","<<best_val<<"]"<<std::endl;
