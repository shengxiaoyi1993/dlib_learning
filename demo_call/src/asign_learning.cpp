...
/// 使用机器学习解决分配问题
/// 一般使用匈牙利算法。但是要求提供组成比例？
/// 下面的例子，展示如何学习一个组成函数 -监督机器模型
/// 对象跟踪的项目：learning_to_track_ex.cpp
///
...
/// 匹配的目标：两个集合，各从一个集合中找到一个元素形成匹配
/// 算法的目标是找到最优的分配
/// 下面的是描述一个问题的几种解决方案：每个解决方案描述了左侧集合和右侧集合后提供一个分配方案
/// 下面的代码学习分配方式，能够为还未存在的问题提供分配方案
...
int main()
{
    try
    {
        ...
        /// structural_assignment_trainer<> 的模板元素的要求
        /// feature_extractor::feature_vector_type
        /// feature_extractor::lhs_element
        /// feature_extractor::rhs_element
        /// .get_features
        /// fe_.num_features()
        ///
        /// 模板类:feature_extractor_track_association<detection_type>
        /// detection_type的要求
        /// detection_type::track_type
        /// detection_type::track_type::feature_vector_type
        ...
        // test by test data
        {
            ...
            std::vector<label_type> test_labels;
            make_test_data(test_samples,test_labels);

            for(unsigned long i=0;i<test_samples.size();i++)
            {
                std::vector<long> predicted_assignments = assigner(test_samples[i]);
                cout << "expected labels:  " << trans(mat(test_labels[i]));
                cout << "predicted labels: " << trans(mat(predicted_assignments)) << endl;
            }
        }
...
)
{
    ...
    mapping.resize(3);
    mapping[0] = 0;  // lhs[0] matches rhs[0]
    mapping[1] = 1;  // lhs[1] matches rhs[1]
    mapping[2] = 2;  // lhs[2] matches rhs[2]
