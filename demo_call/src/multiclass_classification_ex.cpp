...
int main()
{
    try
    {
        ...
        /// 产生训练数据
        /// samples是由三堆点
        /// labels是三堆点的标识，说明某个点属于哪一堆
        generate_data(samples, labels);

        for(auto &it:samples)
        {
//            std::cout<<"["<<it<<"]";
            std::cout<<"["<<it(0,0)<<","<<it(1,0)<<"]";
...
//        img.set_size(2000,2000);
//        for(auto &it:samples)
//        {
//            std::cout<<"["<<it(0,0)<<","<<it(1,0)<<"]";
//            img[it(0,0)*20+1000][it(1,0)*20+1000]=rgb_pixel(233,255,255);
...
//        my_window2.wait_until_closed();
        ...
        /// - 岭回归算法和RBF
        /// - 向量机和多项式
        /// 这展示是的是one_vs_one_trainer可以使用任何二分类器
        ...
        */
        ...
        ///多分类器决策函数不知道它包含了哪些二元分类函数。因此需要使用any_cast
        /// 如果any_cast 到一个错误的二元分类器类型，会抛出异常
    }
